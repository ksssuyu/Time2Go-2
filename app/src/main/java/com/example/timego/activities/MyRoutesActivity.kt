...
class MyRoutesActivity : AppCompatActivity() {
    ...
    companion object {
        private const val TAG = "MyRoutesActivity"
    }
    ...
    private fun loadMyRoutes() {
        ...
        lifecycleScope.launch {
            try {
                repository.getMyRoutes(userId, 100).onSuccess { routes ->
                    displayRoutes(routes)
                }.onFailure { error ->
                    Log.e(TAG, "Ошибка загрузки маршрутов", error)
                    Toast.makeText(
                        this@MyRoutesActivity,
                ...
                Log.e(TAG, "Ошибка", e)
                Toast.makeText(this@MyRoutesActivity, "Произошла ошибка", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun displayRoutes(routes: List<Route>) {
        if (routes.isEmpty()) {
            tvRoutesCount.text = "У вас пока нет маршрутов"
            rvRoutes.visibility = View.GONE
            emptyState.visibility = View.VISIBLE
            return
        }

        tvRoutesCount.text = "Моих маршрутов: ${routes.size}"
